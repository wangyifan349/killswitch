...
pool = ThreadPoolExecutor(max_workers=50)  # 设置合适的线程数量，# 创建线程池

def check_bridge(bridge):# 定义任务函数
...
for bridge in unique_bridges:# 提交任务给线程池
    pool.submit(check_bridge, bridge)

pool.shutdown(wait=True)# 等待所有任务完成

sorted_available_bridges = sorted(available_bridges, key=extract_port)# 按端口号排序
...
with open("bridges.txt", "w") as file:# 打开 bridges.txt 文件进行写入，覆盖原有内容
