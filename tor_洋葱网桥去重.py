...
def extract_port(bridge):#提取端口号，端口号的前面有冒号，后面是数字。
...
def extract_ip(bridge):
    match = re.search(r'(\d+\.\d+\.\d+\.\d+)', bridge)#提取ip4的规则
    if match:
...
sorted_bridges = sorted_available_bridges + ['-----------------------------------------------------'] + unavailable_bridges#上面的网桥可以直连。
